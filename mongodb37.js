...
ANS: db.product.find({"product_price":{$gte:400,$lte:600}});
...
ANS:db.product.find({ "product_material": /soft/i })

9 .Find products which contain product color indigo  and product price 492.00

ANS:db.product.find({"product_color":"indigo","product_price":492.00}) 
# at the product_price of 492.00 the product_color indigo is not there

10 .Delete the products which product price value are same

ANS: var cursor = db.product.aggregate([
    {
      $group: {
  ...
  cursor.forEach(function(doc) {
    doc.uniqueIds.shift(); 
    db.product.deleteMany({ _id: { $in: doc.uniqueIds } });
  });
